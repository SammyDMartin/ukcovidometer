...
def project_vacc(dates,data,daterange):
...
def check_for_percent_vacc(dates_entire,data_entire, percent):
    date_range = pd.date_range(start="2020-12-1",end="2021-12-01").to_pydatetime().tolist()
    vacc_pred,_ = project_vacc(dates_entire,data_entire,date_range)
...
def Vaccine(n):
    ...
    date_range = pd.date_range(start="2020-12-1",end="2021-06-01").to_pydatetime().tolist()
    ...
    dates_daily,data_daily = daily_vacc()
    ...
    vacc_pred,daily_rate = project_vacc(dates_entire,data_entire,date_range)

    phase0 = 15e6/uk_pop
    phase1 = 32e6/uk_pop

    everyone = check_for_percent_vacc(dates_entire,data_entire,1.0)
    full_herd = check_for_percent_vacc(dates_entire,data_entire,0.8)
    partial_herd = check_for_percent_vacc(dates_entire,data_entire,phase1)
    prior = check_for_percent_vacc(dates_entire,data_entire,phase0)
...
if __name__ == "__main__":
    ...
    try:
        with HiddenPrints():
            ...
            Vaccine(14)
            Vaccine(30)
        vaccine_ratio = Vaccine(7)
    except Exception as E:
        print("Vaccine extract fail!")
        print(E)
        vaccine_ratio = 0.5
